...
**print_summary(result: dict) -> None**
- 引数: result - get_switch_dataから取得したデータの辞書
- 戻り値: なし
- 説明: スイッチ情報の概要を人間が読みやすい形式で表示
  - スイッチモデルと説明
  - ポート状態（UP/DOWN、速度、デュプレックス）
  - ポート稼働率
  - VLAN設定一覧
  - MACアドレステーブル統計
  - ポート別MAC数
...
任意の有効なスイッチデータに対して、`print_summary`関数は以下の情報を人間が読みやすい形式で表示すべきである：
- スイッチの基本情報（モデル、説明）
- 各ポートの状態（UP/DOWN、速度、デュプレックス）
- ポート稼働率の計算
- VLAN設定の一覧
- MACアドレステーブルの統計
- ポート別のMAC数集計
